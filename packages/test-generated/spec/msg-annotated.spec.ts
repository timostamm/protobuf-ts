import {
  AnnotatedMessage as AnnotatedMessage_Speed,
  FieldUiBehaviour as FieldUiBehaviour_Speed,
} from "../ts-out/speed/msg-annotated";
import {
  AnnotatedMessage as AnnotatedMessage_Size,
  FieldUiBehaviour as FieldUiBehaviour_Size,
} from "../ts-out/size/msg-annotated";
import {
  AnnotatedMessage as AnnotatedMessage_SpeedBigInt,
  FieldUiBehaviour as FieldUiBehaviour_SpeedBigInt,
} from "../ts-out/speed-bigint/msg-annotated";
import {
  AnnotatedMessage as AnnotatedMessage_SizeBigInt,
  FieldUiBehaviour as FieldUiBehaviour_SizeBigInt,
} from "../ts-out/size-bigint/msg-annotated";
import { readFieldOption, readMessageOption } from "@protobuf-ts/runtime";

const msgs = {
  speed: {
    annotatedMessage: AnnotatedMessage_Speed,
    fieldUiBehaviour: FieldUiBehaviour_Speed,
  },
  size: {
    annotatedMessage: AnnotatedMessage_Size,
    fieldUiBehaviour: FieldUiBehaviour_Size,
  },
  speedBigInt: {
    annotatedMessage: AnnotatedMessage_SpeedBigInt,
    fieldUiBehaviour: FieldUiBehaviour_SpeedBigInt,
  },
  sizeBigInt: {
    annotatedMessage: AnnotatedMessage_SizeBigInt,
    fieldUiBehaviour: FieldUiBehaviour_SizeBigInt,
  },
};

Object.entries(msgs).forEach(
  ([name, { annotatedMessage, fieldUiBehaviour }]) => {
    describe("readFieldOption " + name, function () {
      it("should read scalar opt", function () {
        let act = readFieldOption(
          annotatedMessage,
          "annScalar",
          "spec.opt_string"
        );
    ...
    describe("readMessageOption " + name, function () {
      it("should read scalar opt", function () {
    ...
    describe("spec.AnnotatedMessage", function () {
      ...
      it("field ann_repeated_scalar should have repeated scalar options", function () {
        let fi = annotatedMessage.fields.find(
          (fi) => fi.name === "ann_repeated_scalar"
        );
        expect(fi).toBeDefined();
        if (fi) {
          expect(fi.options).toBeDefined();
          if (fi.options) {
            ...
            expect(fi.options["spec.opt_repeated_uint64"]).toEqual([
              "123456",
              "789101112",
            ]);
            expect(fi.options["spec.opt_repeated_string"]).toEqual([
              "hello...",
              "...world",
            ]);
          }
      ...
      it("field ann_enum_zero should have enum option", function () {
        let fi = annotatedMessage.fields.find(
          (fi) => fi.name === "ann_enum_zero"
        );
        expect(fi).toBeDefined();
        if (fi) {
          expect(fi.options).toBeDefined();
          if (fi.options) {
            expect(fi.options["spec.opt_enum"]).toEqual(
              "OPTION_ENUM_UNSPECIFIED"
            );
          }
        }
      });
      ...
      it("field ann_repeated_enum should have repeated enum option", function () {
        let fi = annotatedMessage.fields.find(
          (fi) => fi.name === "ann_repeated_enum"
        );
        expect(fi).toBeDefined();
        if (fi) {
          expect(fi.options).toBeDefined();
          if (fi.options) {
            expect(fi.options["spec.opt_repeated_enum"]).toEqual([
              "OPTION_ENUM_YES",
              "OPTION_ENUM_NO",
            ]);
          }
      ...
      it("field user_name should have message option", function () {
        ...
        let act = annotatedMessage.fields.find((fi) => fi.name === "user_name")
          ?.options?.["spec.field_ui"];
