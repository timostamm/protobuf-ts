...
Object.entries(msgs).forEach(([name, msgs]) => {
  describe("reflectionCreate() " + name, function () {
    Object.values(msgs).forEach((msg) => {
      describe(`with message type ${msg.typeName}`, () => {
        it("creates the same object as create()", function () {
          const message = reflectionCreate(msg);
          expect(message).toEqual(msg.create());
        });
      });
    });
  });
...
const testAllTypes = {
  speed: () => reflectionCreate(TestAllTypesProto3_Speed),
  size: () => reflectionCreate(TestAllTypesProto3_Size),
  speedBigInt: () => reflectionCreate(TestAllTypesProto3_SpeedBigInt),
  sizeBigInt: () => reflectionCreate(TestAllTypesProto3_SizeBigInt),
};

Object.entries(testAllTypes).forEach(([name, func]) => {
  describe("TestAllTypesProto3 " + name, function () {
