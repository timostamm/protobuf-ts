...
export class ReflectionJsonWriter {
    ...
    if (field.kind == 'map') {
      ...
      switch (field.V.kind) {
        ...
        case "enum":
          ...
          for (const [entryKey, entryValue] of Object.entries(value)) {
            assert(entryValue === undefined || typeof entryValue == 'number' || typeof entryValue == 'string');
      ...
      switch (field.kind) {
        ...
        case "enum":
          ...
          for (let i = 0; i < value.length; i++) {
            assert(value[i] === undefined || typeof value[i] == 'number' || typeof value[i] == 'string');
  ...
  enum (type: EnumInfo, value: unknown, fieldName: string, optional: boolean, emitDefaultValues: boolean, enumAsInteger: boolean): JsonValue | undefined {
    ...
    const isDefault = (value === 0 || type[1][value] === 0);
    if (isDefault && !emitDefaultValues && !optional) {
      // we require a custom default value or 0 to be default value for all enums
      return undefined;
    }
    if (enumAsInteger) {
    ...
    if (typeof value == 'number') {
      ...
      // since value is int, map int -> string.
      return type[1][value];
    }
    if (!type[1].hasOwnProperty(value)) {
      // if we don't now the enum value, just return the string.
      return value;
    }
    if (type[2])
