import { assert, EnumInfo, listEnumNames } from ".";
    ...
    while (reader.pos < end) {
      ...
      switch (field.kind) {
        ...
        case "enum": {
          ...
          if (repeated) {
            let arr = target[localName] as any[];
            if (wireType == WireType.LengthDelimited) {
              let e = reader.uint32() + reader.pos;
              while (reader.pos < e) {
                arr.push(enumNames[reader.int32()]);
              }
            } else {
              arr.push(enumNames[reader.int32()]);
            }
          } else {
            target[localName] = enumNames[reader.int32()];
          }
          break;
        }
        case "message":
        ...
        case 2:
          switch (field.V.kind) {
            ...
            case "enum":
              ...
              const enumValue = enumNames[reader.int32()]
              val = enumValue;
              break;
            case "message":
