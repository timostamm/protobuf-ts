...
import { listEnumNames } from "./enum-object";
  ...
  write<T extends object> (message: T, writer: IBinaryWriter, options: BinaryWriteOptions): void {
    ...
    for (const field of this.fields!) {
      ...
      switch (field.kind) {
        ...
        case "enum":
          let T = ScalarType.INT32;
          const enumNames = listEnumNames(field.T()[1])
          if (repeated) {
            assert(Array.isArray(value));
            if (repeated == RepeatType.PACKED) {
              const intEnums = []
              for (let val of value) {
                intEnums.push(enumNames.indexOf(val.toString()))
              }
              this.packed(writer, T, field.no, intEnums);

            } else {
              for (const item of value)
                this.scalar(writer, T, field.no, enumNames.indexOf(item.toString()), true);
            }
          } else if (value === undefined) {
            assert(field.opt);
          }
          else {
            this.scalar(writer, T, field.no, enumNames.indexOf(value.toString()), emitDefault || field.opt);
          }
          break;
  ...
  protected mapEntry (writer: IBinaryWriter, options: BinaryWriteOptions, field: FieldInfo & { kind: 'map' }, key: any, value: any): void {
    ...
    switch (field.V.kind) {
      case 'scalar':
        ...
        break;
      case 'enum':
        const enumNames = listEnumNames(field.V.T()[1])
        this.scalar(writer, ScalarType.INT32, 2, enumNames.indexOf(value), true);
        break;
      case 'message':
