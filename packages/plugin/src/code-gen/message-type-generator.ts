...
export class MessageTypeGenerator extends GeneratorBase {
  ...
  private readonly symbolKindClass = 'message-class';
  ...
  registerSymbols (source: TypescriptFile, descriptor: DescriptorProto): void {
    ...
    const className = `${basename}\$Type`;
    this.symbols.register(className, descriptor, source, this.symbolKindClass);
  }
  ...
  generateMessageType (source: TypescriptFile, descriptor: DescriptorProto, optimizeFor: OptimizeMode): void {
    ...
    this.registerSymbols(source, descriptor)
    ...
    // export class "MyMessage$Type" extends "MessageType"<"MyMessage"> {
    const classDec = ts.createClassDeclaration(
      undefined, [ts.createModifier(ts.SyntaxKind.ExportKeyword)], Message$Type, undefined,
