...
class FixtureRegistry {
  ...
  /*addMappingsToEnumFields (fields: PartialFieldInfo[]): PartialFieldInfo[] {
    // Adds mappings to enum fields' info.
    let mappedFields: PartialFieldInfo[] = []
    for (let field of fields) {
      if (field.kind == 'enum') {
        // Add mapping from int to string to enumInfo.
        const enumNames = Object.keys(field.T()[1]);
        const values: EnumObjectValue[] = [];
        for (let number = 0; number < enumNames.length; number++) {
          const name = enumNames[number]
          values.push({ name, number });
        }
        let object: EnumInfo[1] = {};
        for (let v of values) {
          object[v.number] = v.name;
          object[v.name] = v.number;
        }
        assert(isEnumObject(object))
        ...
        const enumField = {
          no: field.no, name: field.name, localName: field.localName, jsonName: field.jsonName, repeat: field.repeat,
          opt: field.opt, oneof: field.oneof, options: field.options, kind: field.kind, T: () => { return enumInfo; }
        } as PartialFieldInfo;

        mappedFields.push(enumField);
      } else {
