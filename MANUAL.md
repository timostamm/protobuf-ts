...
The generated code has no dependencies besides the runtime (@chippercash/protobuf-runtime) and 
strictly conforms to the protobuf spec. 

The available packages are listed [here](./packages/) - but you probably want to 
start with the plugin `@chippercash/protobuf-plugin`. 
...
# with yarn:
yarn add @chippercash/protobuf-plugin
```
...
import {AnnotatedService} from "./service-annotated";
import {readMethodOption} from "@chippercash/protobuf-runtime-rpc";
import {JsonValue} from "@chippercash/protobuf-runtime";
...
|-------------|---------------------------------------|-----------------------------------------------------|
| Messages    | `AnnotatedMessage.options`            | `readMessageOption()` from @chippercash/protobuf-runtime     |
| Fields      | `AnnotatedMessage.field[0].options`   | `readFieldOption()` from @chippercash/protobuf-runtime       |
| Services    | `AnnotatedService.options`            | `readServiceOption()` from @chippercash/protobuf-runtime-rpc |
| Methods     | `AnnotatedService.methods[0].options` | `readMethodOption()` from @chippercash/protobuf-runtime-rpc  |
...
- `TwirpFetchTransport` from `@chippercash/protobuf-twirp-transport` - see [Twirp transport](#twirp-transport)
- `GrpcWebFetchTransport` from `@chippercash/protobuf-grpcweb-transport` - see [gRPC web transport](#grpc-web-transport)
- `GrpcTransport` from `@chippercash/protobuf-grpc-transport` - see [gRPC transport](#grpc-transport)
...
`@chippercash/protobuf-runtime-rpc` provides implementations for each of the four method types, 
...
To use the gRPC web transport, install the package `@chippercash/protobuf-grpcweb-transport`.
...
To use the Twirp transport, install the package `@chippercash/protobuf-twirp-transport`.
...
To use the gRPC transport, install the package `@chippercash/protobuf-grpc-transport`.
...
2. install all related packages with `npm i @chippercash/protobuf-runtime @chippercash/protobuf-runtime-rpc @chippercash/protobuf-runtime-angular @chippercash/protobuf-twirp-transport`
...
from @chippercash/protobuf-grpcweb-transport:
